use actix_web::{App, Result, Responder, web::{self, Data}, dev::{ServiceFactory, ServiceRequest}, HttpResponse, get};
    ...
    T: ServiceFactory<ServiceRequest, Config = (), Error = actix_web::Error, InitError = ()>,
{
    ...
    app.app_data(Data::new(archive)).service(handle_index).service(handle_file)
}

#[get("/jumperlab")]
async fn handle_index(data: web::Data<Arc<Mutex<ZipArchive<Cursor<&[u8]>>>>>) -> Result<impl Responder> {
    serve_path("index.html", data)
}

#[get("/jumperlab/{path}*")]
async fn handle_file(path: web::Path<String>, data: web::Data<Arc<Mutex<ZipArchive<Cursor<&[u8]>>>>>) -> Result<impl Responder> {
    serve_path(path.as_str(), data)
}

fn serve_path(path: &str, data: web::Data<Arc<Mutex<ZipArchive<Cursor<&[u8]>>>>>) -> Result<impl Responder> {
