...
const SPECIAL_NETS: [(u8, &'static str, Node); 7] = [
    (1, "GND", Node::GND),
    (2, "+5V", Node::SUPPLY_5V),
    ...
    for (index, name, node) in &SPECIAL_NETS {
        if let Some(net) = by_index.get(index) {
            if net.name.as_str() != *name {
                return Err(anyhow::anyhow!("Special net {} (index: {}) cannot be renamed", name, index));
            }
            if !net.nodes.contains(node) {
                return Err(anyhow::anyhow!("Special net {} (index: {}) is missing node {:?}", name, index, node));
            }
        } else {
            return Err(anyhow::anyhow!("Special net {} (index: {}) missing", name, index));
        }
    }
